...
function basculeEcran(sens) {    
    ...
    var lecteurDonnees = new GeoExt.data.FeatureReader({
        fields: [{name: 'st_asgeojson'},
          ...
          {name:'denombrementmin'},
          {name:'denombrementmax'},
          {name:'type_effectif'},
          {name:'phenologie'},
          {name:'determination'},
          {name:'localisation'},
    ...
    var filtres = new Ext.ux.grid.GridFilters({
        ...
        filters: [
            {type: 'string', dataIndex: 'statut_observation', emptyText: 'Ex. : Val1||Val2||Val3'},
            ...
            {type: 'string', dataIndex: 'datedebut', emptyText: 'Ex. : Val1||Val2||Val3'},
            {type: 'string', dataIndex: 'datefin', emptyText: 'Ex. : Val1||Val2||Val3'},
            ...
            {type: 'string', dataIndex: 'denombrementmin', emptyText: 'Ex. : Val1||Val2||Val3'},
            {type: 'string', dataIndex: 'denombrementmax', emptyText: 'Ex. : Val1||Val2||Val3'},
            ...
            {type: 'string', dataIndex: 'commentaire', emptyText: 'Ex. : Val1||Val2||Val3'},
    ...
    var configCols = new Ext.MyColumnModel({
        ...
        columns: [
            ...
            {dataIndex: 'commentaire', header: 'Remarque'},
            {dataIndex: 'code_insee', header: 'Code insee'},
...
function filterPanelForm() {
  ...
  var comboCommune = createFilterCombobox('filter_commune', 'Commune',
    '../Modeles/GeoJson/gjTableValue.php?table=grp_thematique_obs_occ&schema=synthese_faune&column=commune&limit=200');
  var comboZoneBiogeo = createFilterCombobox('filter_zone_biogeo', 'Zone biogéographique',
    '../Modeles/GeoJson/gjTableValue.php?table=grp_thematique_obs_occ&schema=synthese_faune&column=zone_biogeo');
    
    
  var ckbfilteremprise = new Ext.form.CheckboxGroup({
    id:'ckbfilteremprise',
    xtype: 'checkboxgroup',
    fieldLabel: '',
    itemCls: 'x-check-group-alt',
    ...
    columns: 1,
    items: [
        {boxLabel: 'Filtrer sur l\'emprise', name: 'cb-filteremprise'},
    ]
  });
  var locationFormFielSet = [{
      id : 'localisation',
      xtype: 'fieldset',
      title: 'Localisation',
      autoHeight: true,
      ...
      collapsible: true,
      items: [ckbfilteremprise,comboCommune , comboZoneBiogeo]
    }];
  ...
  var dateFieldSet = [{
   ...
   var comboValidation = createFilterCombobox('filter_statut_validation', 'Validation',
      '../Modeles/GeoJson/gjTableValue.php?table=grp_thematique_obs_occ&schema=synthese_faune&column=statut_validation');
      //Combo d'auto-complétion "exposition"
   var comboSource = createFilterCombobox('filter_source', 'Source',
      '../Modeles/GeoJson/gjTableValue.php?table=grp_thematique_obs_occ&schema=synthese_faune&column=source');
  ...
  var fp = new Ext.FormPanel({
      ...
      collapsible: true,
      autoScroll:true,
      items: [locationFormFielSet, gptFieldSet, dateFieldSet, otherFormFielSet],
      buttons: [{
        text: 'Filtrer',
         handler: filterForm,
      },{
 ...
 if(fp.getForm().isValid()){
    ...
    for(var id in filterValues){
      if (id.substring(0,4) === 'gpt_' ) {
        ...
        if (id === 'startdt' )  {
          sqlFiltres.push(" datedebut >= '"+ filterValues[id] +"'" );
        }
        else if  (id === 'enddt' )  {
          sqlFiltres.push(" datedebut <= '"+ filterValues[id] +"'" );
        }
        else if (id === 'cb-filteremprise') {
          var emprise = carte.getExtent().toGeometry().transform(carte.getProjectionObject(), new OpenLayers.Projection('EPSG:4326'));
          sqlFiltres.push(" st_intersects(centroid ,  st_transform(ST_GeometryFromText('" + emprise +"', 4326),2154))"  );
        }
